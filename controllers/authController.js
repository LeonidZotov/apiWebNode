...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(new AppError('empty email or pass', 400));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('no such usser', 404));
  }
  ...
  const message = `Forgot your pass.\n GO link: ${resetURL}`;

  try {
    await sendEmail({
      email: user.email,
      subject: 'Token valid for 10 min',
      message,
    });
    res.status(201).json({
      status: 'success',
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('No such user', 400));
